...
It could be useful to split your reference data by chromosome. Sometimes it is
required to perform gene-based association tests one chromosome at a time due to
the limited amount of memory available on computers. Intuitively, as more
redundant SNPs are included in the reference data, you tests will consume more
memory and get slower. (For instance, when you are testing genes on chromosome
1, you don't have to consider genotypes on other chromosomes). Although
genotypes are retrieved on demand to efficiently handle large genotype data,
loading information for > 10M markers (in .bim file) into R at once can be
demanding.
...
genotype data to R session. Genotypes are retrieved only on demand to manage
